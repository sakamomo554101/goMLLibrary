...
func TestSGD(t *testing.T) {
	Convey("Given : 1つの重み、1つのバイアスが与えられた時", t, func() {
		...
		w := mat.NewDense(4, 3, createFloatArrayByStep(12, 1.0, 1.0))
		dw := mat.NewDense(4, 3, createFloatArrayByStep(12, 0, 0.5))
		params["w"] = w
		grads["w"] = dw
		Convey("AND バイアスは3次元で各値は0-2とし、勾配は-2-0とする", nil)
		b := mat.NewVecDense(3, createFloatArrayByStep(3, 0, 1.0))
		db := mat.NewVecDense(3, createFloatArrayByStep(3, -2, 1.0))
		params["b"] = b
		grads["b"] = db
